...
//				
					...
					public String search(@RequestParam(value="debitnote", required=false) 
										 org.fenixedu.treasury.domain.document.DebitNote 
					 debitNote
					,@RequestParam(value="payordebtaccount", required=false) 
					...
					,@RequestParam(value="state", required=false) 
										org.fenixedu.treasury.domain.document.FinantialDocumentStateType 
										  state
					, Model model) {
							List<CreditNote> searchcreditnoteResultsDataSet = filterSearchCreditNote(   debitNote  ,   payorDebtAccount  ,   finantialDocumentType  ,   debtAccount  ,   documentNumberSeries  ,   currency  ,   documentNumber  ,   documentDate  ,   documentDueDate  ,   originDocumentNumber  ,   state  );
						...
						model.addAttribute("CreditNote_finantialDocumentType_options", new ArrayList<org.fenixedu.treasury.domain.document.FinantialDocumentType>()); // CHANGE_ME - MUST DEFINE RELATION
						...
						model.addAttribute("CreditNote_documentNumberSeries_options", new ArrayList<org.fenixedu.treasury.domain.document.DocumentNumberSeries>()); // CHANGE_ME - MUST DEFINE RELATION
				...
				private List<CreditNote> getSearchUniverseSearchCreditNoteDataSet() {
					...
					//
					//
					// return new ArrayList<CreditNote>(CreditNote.findAll().collect(Collectors.toList())); //CHANGE_ME
		...
		, org.fenixedu.treasury.domain.document.FinantialDocumentStateType state 
			...
			return getSearchUniverseSearchCreditNoteDataSet().stream()
				...
				.filter(creditNote -> finantialDocumentType == null || finantialDocumentType == creditNote.getFinantialDocumentType())					
				.filter(creditNote -> debtAccount == null || debtAccount == creditNote.getDebtAccount())					
				.filter(creditNote -> documentNumberSeries == null || documentNumberSeries == creditNote.getDocumentNumberSeries())					
				.filter(creditNote -> currency == null || currency == creditNote.getCurrency())					
				.filter(creditNote -> documentNumber == null || 
					 documentNumber.length() == 0 ||  (creditNote.getDocumentNumber() != null && creditNote.getDocumentNumber().length() > 0 && creditNote.getDocumentNumber().toLowerCase().contains(documentNumber.toLowerCase())))
				...
				.filter(creditNote -> originDocumentNumber == null || 
					 originDocumentNumber.length() == 0 ||  (creditNote.getOriginDocumentNumber() != null && creditNote.getOriginDocumentNumber().length() > 0 && creditNote.getOriginDocumentNumber().toLowerCase().contains(originDocumentNumber.toLowerCase())))
				.filter(creditNote -> state == null || state.equals(creditNote.getState()))
		...
		return redirect("/treasury/document/manageinvoice/creditnote/read" + "/" + creditNote.getExternalId(), model, redirectAttributes);
		}
...
//				
				...
				public String create(@RequestParam(value="debitnote", required=false) 
									 org.fenixedu.treasury.domain.document.DebitNote 
				 debitNote
				,@RequestParam(value="payordebtaccount", required=false) 
									 org.fenixedu.treasury.domain.debt.DebtAccount 
				...
				, Model model, RedirectAttributes redirectAttributes ) {
					...
					*/
					...
					try
					{
				     	...
				     	CreditNote creditNote = createCreditNote(   debitNote  ,   payorDebtAccount  ,   finantialDocumentType  ,   debtAccount  ,   documentNumberSeries  ,   currency  ,   documentNumber  ,   documentDate  ,   documentDueDate  ,   originDocumentNumber  ,   state  );
...
return redirect("/treasury/document/manageinvoice/creditnote/read/" + getCreditNote(model).getExternalId(), model, redirectAttributes);
	...
	// @formatter: on
						...
						CreditNote creditNote = CreditNote.create(debitNote, payorDebtAccount, finantialDocumentType, debtAccount, documentNumberSeries, currency, documentNumber, documentDate, documentDueDate, originDocumentNumber, state);
						 return creditNote;
				}
//				
...
//				
				...
				public String update(@PathVariable("oid") CreditNote creditNote, @RequestParam(value="debitnote", required=false) 
									 org.fenixedu.treasury.domain.document.DebitNote 
				 debitNote
				,@RequestParam(value="payordebtaccount", required=false) 
									 org.fenixedu.treasury.domain.debt.DebtAccount 
				...
				, Model model, RedirectAttributes redirectAttributes) {
					...
					*/
						...
						updateCreditNote( debitNote ,  payorDebtAccount ,  finantialDocumentType ,  debtAccount ,  documentNumberSeries ,  currency ,  documentNumber ,  documentDate ,  documentDueDate ,  originDocumentNumber ,  state ,  model);
...
return redirect("/treasury/document/manageinvoice/creditnote/read/" + getCreditNote(model).getExternalId(), model, redirectAttributes);
				...
				@Atomic
				public void updateCreditNote(  org.fenixedu.treasury.domain.document.DebitNote debitNote 
				, org.fenixedu.treasury.domain.debt.DebtAccount payorDebtAccount 
				, org.fenixedu.treasury.domain.document.FinantialDocumentType finantialDocumentType 
				, org.fenixedu.treasury.domain.debt.DebtAccount debtAccount 
				, org.fenixedu.treasury.domain.document.DocumentNumberSeries documentNumberSeries 
