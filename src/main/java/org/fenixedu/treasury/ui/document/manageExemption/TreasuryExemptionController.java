				...
				, Model model, RedirectAttributes redirectAttributes ) {
					...
					*/
					...
					try
					{
				     	...
				     	TreasuryExemption treasuryExemption = createTreasuryExemption(   code  ,   name  ,   discountRate  );
				    	
					//Success Validation
				     //Add the bean to be used in the View
					model.addAttribute("treasuryExemption",treasuryExemption);
return redirect("/treasury/accounting/managecustomer/customer/read/" + getTreasuryExemption(model).getExternalId(), model, redirectAttributes);
					}
	...
	// @formatter: off
						 ...
						 */
						 ...
						 //TreasuryExemption treasuryExemption = treasuryExemption.create(fields_to_create);
						 
						 //Instead, use individual SETTERS and validate "CheckRules" in the end
	// @formatter: on

						TreasuryExemption treasuryExemption = TreasuryExemption.create(code, name, discountRate);
...
//				
					...
					,@RequestParam(value="discountrate", required=false) 
										java.math.BigDecimal 
										  discountRate
					, Model model) {
							List<TreasuryExemption> searchtreasuryexemptionResultsDataSet = filterSearchTreasuryExemption(   code  ,   name  ,   discountRate  );
						
						//add the results dataSet to the model
						model.addAttribute("searchtreasuryexemptionResultsDataSet",searchtreasuryexemptionResultsDataSet);
		...
		, java.math.BigDecimal discountRate 
			...
			return getSearchUniverseSearchTreasuryExemptionDataSet().stream()
				.filter(treasuryExemption -> code == null || 
					 code.length() == 0 ||  (treasuryExemption.getCode() != null && treasuryExemption.getCode().length() > 0 && treasuryExemption.getCode().toLowerCase().contains(code.toLowerCase())))
				.filter(treasuryExemption -> name == null
					|| name.isEmpty()
					|| name.getLocales()
							.stream()
							.allMatch(
									locale -> treasuryExemption.getName().getContent(locale) != null
											&& treasuryExemption.getName().getContent(locale)
													.toLowerCase()
													.contains(name.getContent(locale).toLowerCase())))
				.filter(treasuryExemption -> discountRate == null || discountRate.equals(treasuryExemption.getDiscountRate()))
				.collect(Collectors.toList());				
		}
		
		
				@RequestMapping(value = "/search/view/{oid}")
				public String processSearchToViewAction(@PathVariable("oid") TreasuryExemption treasuryExemption,  Model model, RedirectAttributes redirectAttributes) {
		
			// CHANGE_ME Insert code here for processing viewAction
			// If you selected multiple exists you must choose which one to use below	 
		return redirect("/treasury/document/manageexemption/treasuryexemption/read" + "/" + treasuryExemption.getExternalId(), model, redirectAttributes);
		}
...
//
					...
					public String delete(@PathVariable("oid") TreasuryExemption treasuryExemption, Model model, RedirectAttributes redirectAttributes) {
		...
		return redirect("/treasury/document/manageexemption/treasuryexemption/", model, redirectAttributes);
						}
						catch(DomainException ex)
						{
							//Add error messages to the list
							addErrorMessage("Error deleting the TreasuryExemption due to " + ex.getLocalizedMessage(),model);
						}
						
						//The default mapping is the same Read View
						return "treasury/document/manageexemption/treasuryexemption/read/" + getTreasuryExemption(model).getExternalId();
					}  
//				
...
//				
				...
				public String update(@PathVariable("oid") TreasuryExemption treasuryExemption, @RequestParam(value="code", required=false) 
									java.lang.String 
									  code
				,@RequestParam(value="name", required=false) 
									org.fenixedu.commons.i18n.LocalizedString 
...
return redirect("/treasury/document/manageexemption/treasuryexemption/read/" + getTreasuryExemption(model).getExternalId(), model, redirectAttributes);
					}
					catch (DomainException de) 
					{
						// @formatter: off
