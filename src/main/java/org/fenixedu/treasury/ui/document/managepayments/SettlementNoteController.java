...
//                    }
    ...
    public String calculateInterest(@RequestParam(value = "bean", required = true) SettlementNoteBean bean, Model model) {
        try {
            ...
            for (ISettlementInvoiceEntryBean debitEntryBean : bean.getDebitEntries()) {
                if (debitEntryBean.isIncluded() && debitEntryBean.getInvoiceEntry().getFinantialDocument() == null) {
                    setSettlementNoteBean(bean, model);
