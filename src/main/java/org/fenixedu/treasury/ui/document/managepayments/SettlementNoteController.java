...
public class SettlementNoteController extends TreasuryBaseController {
	...
	for (int i = 0; i < bean.getCreditEntries().size(); i++) {
		...
		// UL-FC-2233
		if (creditEntryBean.getCreditEntry() != null && creditEntryBean.getCreditEntry().getCreditNote() != null
		    ...
		    if(TreasuryConstants.isLessThan(creditEntryBean.getCreditAmountWithVat(), advancedCreditEntry.getOpenAmount())) {
			addWarningMessage(treasuryBundle("warning.CreditEntry.advancedPaymentCredit.creditAmount.less.than.openAmount"), model);
		    }
		}
	    } else {
		creditEntryBean.setNotValid(false);
	...
	if (bean.getPaymentEntries().stream().anyMatch(peb -> TreasuryConstants.isZero(peb.getPaymentAmount()))) {
	    error = true;
	    String errorMessage = bean.isReimbursementNote() ? "error.SettlementNote.reimbursement.equal.zero"
		    : "error.SettlementNote.payment.equal.zero";
	...
	if (bean.isAdvancePayment() && TreasuryConstants.isLessThan(paymentSum, debitSum)) {
	    error = true;
	    final String errorMessage = bean.isReimbursementNote() ? "error.SettlementNote.no.match.reimbursement.credit"
		    : "error.SettlementNote.no.match.payment.debit";
	    addErrorMessage(treasuryBundle(errorMessage), model);
	...
	final List<SettlementNote> result = filterSearch(debtAccount, documentNumber, documentDateFrom, documentDateTo,
	...
	final Stream<SettlementNote> result = debtAccount == null ? SettlementNote.findAll()
		: SettlementNote.findByDebtAccount(debtAccount);
	...
	final boolean allowToConditionallyAnnulSettlementNote = TreasuryAccessControlAPI
		.isAllowToConditionallyAnnulSettlementNote(loggedUsername, settlementNote);

	final boolean allowToAnnulSettlementNoteWithoutAnyRestriction = TreasuryAccessControlAPI
		.isAllowToAnnulSettlementNoteWithoutAnyRestriction(loggedUsername, settlementNote);
	...
	try {
	    ...
	    final String saftEncoding = ERPExporterManager
		    .saftEncoding(settlementNote.getDebtAccount().getFinantialInstitution());
	...
	Map<PaymentMethod, BigDecimal> payments = getPaymentEntriesDataSet(notes).stream()
	...
	Map<PaymentMethod, BigDecimal> reimbursements = getReimbursementEntriesDataSet(notes).stream()
