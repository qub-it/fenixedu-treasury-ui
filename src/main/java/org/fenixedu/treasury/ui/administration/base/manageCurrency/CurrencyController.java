...
@SpringFunctionality(app = TreasuryController.class, title = "label.title.administration.base.manageCurrency",accessGroup = "anyone")// CHANGE_ME accessGroup = "group1 | group2 | groupXPTO"
...
//				
					...
					,@RequestParam(value="name", required=false) 
										org.fenixedu.commons.i18n.LocalizedString 
										  name
					,@RequestParam(value="isocode", required=false) 
										java.lang.String 
										  isoCode
		...
		private List<Currency> filterSearchCurrency( java.lang.String code 
		, org.fenixedu.commons.i18n.LocalizedString name 
		, java.lang.String isoCode 
		, java.lang.String symbol 
		) {
			
			return getSearchUniverseSearchCurrencyDataSet().stream()
				.filter(currency -> code == null || 
					 code.length() == 0 ||  (currency.getCode() != null && currency.getCode().length() > 0 && currency.getCode().toLowerCase().contains(code.toLowerCase())))
					...
					|| name.getLocales()
							...
							.allMatch(
									locale -> currency.getName().getContent(locale) != null
											&& currency.getName().getContent(locale)
													.toLowerCase()
													.contains(name.getContent(locale).toLowerCase())))
				.filter(currency -> isoCode == null || 
					 isoCode.length() == 0 ||  (currency.getIsoCode() != null && currency.getIsoCode().length() > 0 && currency.getIsoCode().toLowerCase().contains(isoCode.toLowerCase())))
				.filter(currency -> symbol == null || 
					 symbol.length() == 0 ||  (currency.getSymbol() != null && currency.getSymbol().length() > 0 && currency.getSymbol().toLowerCase().contains(symbol.toLowerCase())))
		...
		return "redirect:/treasury/administration/base/managecurrency/currency/read" + "/" + currency.getExternalId();
		}
...
//
					...
					public String delete(@PathVariable("oid") Currency currency, Model model) {
		...
		return "redirect:/treasury/administration/base/managecurrency/currency/";
						}
						...
						return "treasury/administration/base/managecurrency/currency/read/" + getCurrency(model).getExternalId();
					}  
//				
...
return "redirect:/treasury/administration/base/managecurrency/currency/read/" + getCurrency(model).getExternalId();
				...
				@Atomic
				public Currency createCurrency( java.lang.String code 
				, org.fenixedu.commons.i18n.LocalizedString name 
				, java.lang.String isoCode 
				, java.lang.String symbol 
				) {
...
//				
				...
				public String update(@PathVariable("oid") Currency currency, @RequestParam(value="code", required=false) 
									java.lang.String 
									  code
				,@RequestParam(value="name", required=false) 
									org.fenixedu.commons.i18n.LocalizedString 
...
return "redirect:/treasury/administration/base/managecurrency/currency/read/" + getCurrency(model).getExternalId();
				...
				@Atomic
				public void updateCurrency(  java.lang.String code 
				, org.fenixedu.commons.i18n.LocalizedString name 
				, java.lang.String isoCode 
				, java.lang.String symbol 
				 ,  Model m) {
