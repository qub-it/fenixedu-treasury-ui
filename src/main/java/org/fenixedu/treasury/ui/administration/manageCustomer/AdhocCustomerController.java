...
//				
		...
		public String processSearchToDeleteMultiple(@RequestParam("adhocCustomers") List<AdhocCustomer> adhocCustomers,  Model model, RedirectAttributes redirectAttributes) {
		...
		return redirect("/treasury/administration/managecustomer/adhoccustomer/", model, redirectAttributes);
			...
			// If you selected multiple exists you must choose which one to use below	 
		return redirect("/treasury/administration/managecustomer/adhoccustomer/read" + "/" + adhocCustomer.getExternalId(), model, redirectAttributes);
		}
		
		
		@RequestMapping(value = "/search/delete/{oid}", method = RequestMethod.POST)
		public String processSearchToDeleteAction(@PathVariable("oid") AdhocCustomer adhocCustomer,  Model model, RedirectAttributes redirectAttributes) {
...
//
		...
		return redirect("/treasury/administration/managecustomer/adhoccustomer/", model, redirectAttributes);
						...
						return "treasury/administration/managecustomer/adhoccustomer/read/" + getAdhocCustomer(model).getExternalId();
					}  
//				
...
//				
				...
				,@RequestParam(value="address", required=false) 
									java.lang.String 
									  address
				,@RequestParam(value="districtsubdivision", required=false) 
									java.lang.String 
									  districtSubdivision
				...
				, Model model, RedirectAttributes redirectAttributes ) {
					...
					*/
					...
					try
					{
				     	...
				     	AdhocCustomer adhocCustomer = createAdhocCustomer(   code  ,   fiscalNumber  ,   name  ,   address  ,   districtSubdivision  ,   zipCode  ,   countryCode  );
				    	
					//Success Validation
				     //Add the bean to be used in the View
					model.addAttribute("adhocCustomer",adhocCustomer);
return redirect("/treasury/administration/managecustomer/adhoccustomer/read/" + getAdhocCustomer(model).getExternalId(), model, redirectAttributes);
					}
				...
				, java.lang.String districtSubdivision 
	...
	// @formatter: on
						...
						AdhocCustomer adhocCustomer = AdhocCustomer.create(code, fiscalNumber, name, address, districtSubdivision, zipCode, countryCode);
