...
<div class="page-header">
	<h1><spring:message code="label.administration.base.manageCurrency.searchCurrency" />
		<small></small>
	</h1>
</div>
<%-- NAVIGATION --%>
<div class="well well-sm" style="display:inline-block">
	<span class="glyphicon glyphicon-plus-sign" aria-hidden="true"></span>&nbsp;<a class="" href="${pageContext.request.contextPath}/treasury/administration/base/managecurrency/currency/create"   ><spring:message code="label.event.create" /></a>
&nbsp;
</div>
	...
	<c:when test="${not empty searchcurrencyResultsDataSet}">
		<table id="searchcurrencyTable" class="table responsive table-bordered table-hover" width="100%">
			<thead>
				<tr>
					<%--!!!  Field names here --%>
<th><spring:message code="label.Currency.code"/></th>
<th><spring:message code="label.Currency.name"/></th>
<th><spring:message code="label.Currency.isoCode"/></th>
<th><spring:message code="label.Currency.symbol"/></th>
<%-- Operations Column --%>
					<th></th>
				</tr>
			</thead>
...
<script>
	var searchcurrencyDataSet = [
			<c:forEach items="${searchcurrencyResultsDataSet}" var="searchResult">
				<%-- Field access / formatting  here CHANGE_ME --%>
				{
				"DT_RowId" : '<c:out value='${searchResult.externalId}'/>',
"code" : "<c:out value='${searchResult.code}'/>",
"name" : "<c:out value='${searchResult.name.content}'/>",
"isocode" : "<c:out value='${searchResult.isoCode}'/>",
"symbol" : "<c:out value='${searchResult.symbol}'/>",
"actions" :
" <a  class=\"btn btn-default btn-xs\" href=\"${pageContext.request.contextPath}/treasury/administration/base/managecurrency/currency/search/view/${searchResult.externalId}\"><spring:message code='label.view'/></a>" +
                "" },
            </c:forEach>
    ];
	...
	$(document).ready(function() {
		...
		var table = $('#searchcurrencyTable').DataTable({language : {
...
//"dom": '<"col-sm-6"l>rtip', // FilterBox = NO && ExportOptions = NO
		...
		table.columns.adjust().draw();
		  ...
		  $('#searchcurrencyTable tbody').on( 'click', 'tr', function () {
