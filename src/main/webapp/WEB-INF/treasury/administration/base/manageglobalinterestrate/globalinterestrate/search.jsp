...
<div class="page-header">
	<h1><spring:message code="label.administration.base.manageGlobalInterestRate.searchGlobalInterestRate" />
		<small></small>
	</h1>
</div>
<%-- NAVIGATION --%>
<div class="well well-sm" style="display:inline-block">
	<span class="glyphicon glyphicon-plus-sign" aria-hidden="true"></span>&nbsp;<a class="" href="${pageContext.request.contextPath}/treasury/administration/base/manageglobalinterestrate/globalinterestrate/create"   ><spring:message code="label.event.create" /></a>
|&nbsp;&nbsp;</div>
...
<div class="tab-pane" id="allGlobalInterestRates">
	<p></p>
	<c:choose>
		<c:when test="${not empty searchglobalinterestrateResultsDataSet}">
			<datatables:table id="allsearchglobalinterestrateResultsTable" row="globalInterestRate" data="${searchglobalinterestrateResultsDataSet}" cssClass="table responsive table-bordered table-hover" cdn="false" cellspacing="2">
				<datatables:column cssStyle="width:10%">
					<datatables:columnHead ><spring:message code="label.GlobalInterestRate.year" /></datatables:columnHead>
					<c:out value="${globalInterestRate.year}" /> 
				</datatables:column>
				<datatables:column cssStyle="width:60%">
					<datatables:columnHead ><spring:message code="label.GlobalInterestRate.description" /></datatables:columnHead>
					<c:out value="${globalInterestRate.description.content}" /> 
				</datatables:column>
				<datatables:column cssStyle="width:10%">
					<datatables:columnHead ><spring:message code="label.GlobalInterestRate.rate" /></datatables:columnHead>
					<c:out value="${globalInterestRate.rate}" />&#37;
				</datatables:column>
				<datatables:column cssStyle="width:20%">
					<a  class="btn btn-default btn-xs" href="${pageContext.request.contextPath}/treasury/administration/base/manageglobalinterestrate/globalinterestrate/search/view/${globalInterestRate.externalId}"><spring:message code="label.view"/></a> 
				</datatables:column>			
			</datatables:table>
	 		<script>
	 		createDataTables('allsearchglobalinterestrateResultsTable',true,false,true,"${pageContext.request.contextPath}","${datatablesI18NUrl}");
	 		</script> 	
		</c:when>
		<c:otherwise>
		<div class="alert alert-warning" role="alert">		
