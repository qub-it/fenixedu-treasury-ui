...
<div class="page-header">
	<h1>
		<spring:message code="label.document.manageInvoice.searchPendingEntries" />
		<small></small>
	</h1>
</div>
<%-- NAVIGATION --%>
...
<table class="table">
	<tbody>
		<tr>
			...
			<td><c:out value='${debitNote.debtAccount.finantialInstitution.name}' /></td>
		</tr>
		<tr>
...
<c:choose>
	<c:when test="${not empty searchpendingentriesResultsDataSet}">
		<table id="searchpendingentriesTable" class="table responsive table-bordered table-hover">
			<thead>
				<tr>
					<%--!!!  Field names here --%>
					<th><spring:message code="label.DebitEntry.entryDateTime" /></th>
					<th><spring:message code="label.DebitEntry.quantity" /></th>
					<th><spring:message code="label.DebitEntry.description" /></th>
					<th><spring:message code="label.DebitEntry.amount" /></th>
		...
		<form id="addentries" action="${pageContext.request.contextPath}/treasury/document/manageinvoice/debitentry/searchpendingentries/addentries?debitNote=${debitNote.externalId}"
			style="display: none;" method="POST"></form>

		<button type="button" onClick="javascript:submitOptions('searchpendingentriesTable', 'addentries', 'debitEntrys')">
			<spring:message code='label.document.manageInvoice.searchPendingEntries.addEntries' />
		</button>
	</c:when>
	<c:otherwise>
		<div class="alert alert-warning" role="alert">
...
"datetime" : "<c:out value='${searchResult.entryDateTime}'/>",
